...
-- MODEL
type alias Bitmask = {mask: String, bits: Int}

-- bits2mask : Bits -> Bitmask
bitMask : Int -> Bitmask
bitMask x = Bitmask (Cidr.bits2mask x |> Cidr.int2ip4) x

bitMasks : List Bitmask
bitMasks = List.map bitMask <| List.reverse <| List.range 1 32
...
view model =
  let
    bitMaskOptions = List.map bitmask2option bitMasks
  in
    div []
      [ text "CIDR Calculator", br,
        input [ placeholder "1.2.3.4", onInput IpUpdated ] [],
        text " with ",
        select [ name "bits", title "bits", onChange BitsUpdated ] ((option [ value "" ] [ text "bits | mask" ]) :: bitMaskOptions),
        br, br,
...
doCidr model =
  if (isValidIp model.ip) && (model.bits >= 0) then
    let
      (low, high) = (cidr model.ip model.bits)
    in
      low ++ " - " ++ high
  else
    "invalid input"

isValidIp str = contains (regex "^(?!\\.)(\\.?[0-9]{1,3}){4}$") str